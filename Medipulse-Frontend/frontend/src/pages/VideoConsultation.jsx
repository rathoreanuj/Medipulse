...
const VideoConsultation = () => {
  ...
  const pendingCandidatesRef = useRef([]) // buffer ICE candidates before remote desc is set
  ...
  // ─── Main setup: verify + get media + connect socket ──────────────────────
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        // 1. Verify appointment and get videoRoomId from backend
        ...
        // Other peer joined → this socket should create the offer
        ...
        socket.on('peer-disconnected', () => { toast.info('The other participant disconnected'); setStatus('ended'); cleanup() })
  ...
  }, [appointmentId, backendUrl, token, navigate, initiateCall, handleOffer, cleanup])
    ...
    return (
      <div className='min-h-[80vh] flex items-center justify-center px-4'>
        <div className='bg-white rounded-xl border border-red-200 p-8 max-w-md w-full text-center'>
          ...
          <button onClick={() => navigate('/my-appointments')} className='bg-primary text-white px-6 py-2.5 rounded-lg hover:bg-blue-600 transition-colors text-sm font-medium'>
    ...
    return (
      <div className='min-h-[80vh] flex items-center justify-center px-4'>
        <div className='bg-white rounded-xl border border-gray-200 p-8 max-w-md w-full text-center'>
          ...
          <p className='text-gray-400 text-xs mb-6'>Thank you for using Medipulse video consultation.</p>
          <button onClick={() => navigate('/my-appointments')} className='bg-primary text-white px-6 py-2.5 rounded-lg hover:bg-blue-600 transition-colors text-sm font-medium'>
            Back to Appointments
          </button>
        </div>
    ...
    <div className='min-h-screen bg-gray-900 flex flex-col'>
      ...
      <div className='flex items-center justify-between px-6 py-4 bg-gray-800'>
        <div className='flex items-center gap-3'>
          ...
          <div>
            <p className='text-white font-semibold text-sm'>{appointment?.doctorName || 'Doctor'}</p>
          ...
          <div className='absolute inset-0 flex items-center justify-center'>
            <div className='text-center'>
              ...
              <p className='text-gray-400 text-sm'>
                {status === 'waiting' ? 'Waiting for the doctor to join…' : 'Setting up connection…'}
              </p>
            </div>
          </div>
        )}
      ...
      <div className='bg-gray-800 px-6 py-5 flex items-center justify-center gap-4'>
        ...
        >
          {isMuted ? (
            ...
            <svg className='w-5 h-5 text-white' fill='none' stroke='currentColor' strokeWidth='2' viewBox='0 0 24 24'>
              <path strokeLinecap='round' strokeLinejoin='round' d='M15.536 8.464a5 5 0 010 7.072M12 18.364a9 9 0 000-12.728M6.343 6.343a8 8 0 000 11.314' />
            </svg>
          )}
        </button>
        ...
        >
          {isCameraOff ? (
            <svg className='w-5 h-5 text-white' fill='none' stroke='currentColor' strokeWidth='2' viewBox='0 0 24 24'>
              <path strokeLinecap='round' strokeLinejoin='round' d='M15 10l4.553-2.069A1 1 0 0121 8.882v6.236a1 1 0 01-1.447.894L15 14M3 8.5A2.5 2.5 0 015.5 6h5A2.5 2.5 0 0113 8.5v7a2.5 2.5 0 01-2.5 2.5h-5A2.5 2.5 0 013 15.5v-7zM3 3l18 18' />
            </svg>
          ) : (
            <svg className='w-5 h-5 text-white' fill='none' stroke='currentColor' strokeWidth='2' viewBox='0 0 24 24'>
